...
impl FlashbotsClient {
    ...
    pub async fn new(
        ...
        info!("✅ Flashbots client initialized (relay: {})", 
            flashbots_relay_url.unwrap_or("https://relay.flashbots.net"));
    ...
    pub async fn send_bundle(&self, transactions: Vec<TypedTransaction>) -> Result<BundleHash> {
        ...
        info!("📦 Creating bundle with {} transactions", transactions.len());
        ...
        for (idx, mut tx) in transactions.into_iter().enumerate() {
            ...
            // Sign transaction
            let signature = self.client.inner()
                .signer()
                .sign_transaction(&tx)
                .await
        ...
        for attempt in 1..=self.max_retries {
            match self.client.send_bundle(&bundle).await {
                Ok(pending_bundle) => {
                    ...
                    info!("✅ Bundle sent! Hash: {:?} (target block: {})", 
                        bundle_hash, target_block);
                    
                    return Ok(BundleHash(bundle_hash));
                }
                Err(e) => {
                    warn!("⚠️ Bundle submission attempt {}/{} failed: {}", 
    ...
    pub async fn simulate_bundle(&self, transactions: Vec<TypedTransaction>) -> Result<SimulationResult> {
        if transactions.is_empty() {
        ...
        match self.client.simulate_bundle(&bundle).await {
            Ok(simulated) => {
                ...
                info!("✅ Simulation SUCCESS - Gas: {}, Price: {}", 
