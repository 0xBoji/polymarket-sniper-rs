...
async fn main() -> anyhow::Result<()> {
    ...
    let signer_address =
        LocalWallet::from_str(&config.polygon_private_key.clone().unwrap())?.address();
    let target_address = if let Some(proxy) = &config.polymarket.proxy_address {
        ...
        println!(
            "🕵️ Using Signer Wallet for Balance Check: {:?}",
            signer_address
        );
        signer_address
    };
    ...
    let bridged_contract = ERC20::new(
        Address::from_str(bridged_usdc).unwrap(),
        Arc::new(provider.clone()),
    );
    let native_contract = ERC20::new(
        Address::from_str(native_usdc).unwrap(),
        Arc::new(provider.clone()),
    );

    // STRICT RPC CHECKS (Panic if network fails)
    let b_balance: U256 = bridged_contract
        .balance_of(target_address)
        .call()
        .await
        .expect("❌ RPC Error: Failed to check Bridged USDC");
    let n_balance: U256 = native_contract
        .balance_of(target_address)
        .call()
        .await
        .expect("❌ RPC Error: Failed to check Native USDC");
    ...
    let market = markets
        ...
        .or_else(|| {
            markets
                .iter()
                .find(|m| m.yes_price > 0.01 && m.yes_price < 0.99)
        })
    ...
    println!(
        "🚀 Placing order: ${:.2} on YES at ${:.4}...",
        size_usd, price
    );

    match executor
        .execute_snipe(market, "YES", price, size_usd, &trade_id, &mut risk_manager)
        .await
    {
        Ok(order_id) => {
